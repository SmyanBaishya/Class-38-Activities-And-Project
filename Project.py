...
levels = [
    ...
    {
        'background': 'level2_background.png',
        'enemy_count': 8,
        'player_start_x': 200,
        'player_start_y': 450,
...
while running:
    ...
    num_of_enemies = level['enemy_count']
    playerX = level['player_start_x']
    playerY = level['player_start_y']
    ENEMY_START_Y_MIN = level['enemy_start_y_min']
    ENEMY_START_Y_MAX = level['enemy_start_y_max']

    # Reset enemy positions for each level
    enemyImg = []
    enemyX = []
    enemyY = []
    enemyX_change = []
    ...
    # Check for level completion and transition to the next level
    if score_value >= 10: # Example level completion condition
        current_level += 1
        score_value = 0 # Reset score for the next level

        # Check if we have reached the last level
        if current_level > len(levels):
            current_level = 1 # Loop back to the first level
